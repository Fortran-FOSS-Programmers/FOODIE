...
contains
  ...
  allocate(error(1:space_dimension, 1:size(self%Dt)))
  error = 0.0_R_P
  if (self%errors_analysis) then
    allocate(order(1:space_dimension, 1:size(error, dim=2)-1))
    order = 0.0_R_P
  endif
  ...
  subroutine integrate(scheme, frequency, final_time, Dt, iterations, stages, is_fast, solution, error, last_step)
  ...
  type is(integrator_adams_bashforth)
     ...
     do while(solution(0, step) < final_time .and. step < ubound(solution, dim=2))
       ...
       if (integrator%steps_number() >= step) then
         domain = exact_solution_xy(frequency=frequency, t=step * Dt)
