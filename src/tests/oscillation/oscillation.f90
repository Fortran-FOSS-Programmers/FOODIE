...
contains
  ...
  do s=1, rk_stages
    ...
    do step = 1, num_steps
      ...
      solution(1:space_dimension, step) = attractor%output()
    enddo
    call save_results(title='FOODiE test: Oscillation equation integration, explicit Runge-Kutta '//trim(str(.true., s))//&
                      ' stages', filename='oscillation_integration-rk-'//trim(str(.true., s)))
  enddo
  print "(A)", 'Finish!'
  return
  !---------------------------------------------------------------------------------------------------------------------------------
