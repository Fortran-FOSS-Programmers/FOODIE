...
\begin{itemize}
  ...
  \item be abstract, thus it is written by means of Object Oriented Programming (OOP) paradigm;
...
One key-point of the FOODIE development is the \emph{problem abstraction}: the problem solved must be the IVP \ref{eq:IVP} rather than any of its real, concrete applications, e.g. the Lorentz system. Consequently, we must rely on a \emph{generic} implementation of the solvers. To this aim, OOP is very useful: it allows to express IVP \ref{eq:IVP} in a very concise and clear formulation that is really generic. In particular, our implementation is based on the \emph{Abstract Calculus Pattern} (ACP) concept.
